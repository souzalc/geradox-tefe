...
class Faleconosco extends CI_Controller {
	...
	function mensagem ($url_de_retorno){
		...
		$assunto = "Contato GeraDox Tefé - ". $subject;
		...
		$mensagem .= '</div>';
		$mensagem .= "GeraDox Tefé- Sistema Gerenciador de Documentos<br />";
	
		$this->load->library('email');
		$this->email->from($email, 'Contato GeraDox Tefé');
	
		$this->email->reply_to($email);
		$this->email->to('lucassectitefe@gmail.com');
